...
- **Hozzáférés:** root/sudo
- **Aktív host:** dn-platform-01 (OpenClaw runtime)

## 1/a. Aktuális 3 VPS architektúra (SSOT)

### Platform VPS
- **Név:** dn-platform-01
- **Szerep:** AI + automation platform (control plane)
- **Docker stack (/opt/platform):** n8n, postgres, imap-proxy, cloudflared
- **Native service-ek:** OpenClaw, kanban, tokenctl, TTS
- **Venv-ek:** /opt/skills-venv, /opt/tts-venv
- **Cloudflare tunnel:** aktív (n8n eléréshez)

### Web VPS
- **Név:** dn-web-01
- **IP:** 46.224.110.90
- **Szerep:** public web node
- **Stack:** Nginx + Astro (jelenleg SSR, cél static)
- **Domain-ek:** vinczetamas.hu, elkezdodott.hu (digitalisninja.hu később)

### Forex VPS
- **Név:** dn-dev-01
- **Szerep:** trading infra
- **Fut:** MT4, EA, trading bot
- **Elkülönítés:** nincs közvetlen platform/web kapcsolat (izolált pénzügyi rendszer)

### DNS/Email
- **DNS provider:** Rackhost
- **vinczetamas.hu A rekord:** 46.224.110.90 (dn-web-01)
- **MX:** tarhely.eu
...
```bash
git clone git@github.com:leonioperator/shared_workspace.git ~/shared_workspace
```

- **Inbox workflow:** Tomi → `~/shared_workspace/inbox/` → git push → Leoni heartbeat-kor pull + feldolgozás
- **Struktúra:**
  ```
  shared_workspace/
  ├── README.md
...
## 9a. Ollama + nomic-embed-text (ChromaDB embedding)

**Telepítve:** 2026-03-14
...
- Service: `ollama` (systemd, auto-start)
- Port: `localhost:11434`
- Model: `nomic-embed-text` (274 MB)
- Használat: learn skill ChromaDB embedding (`/opt/skills-venv`)
- CPU-only (nincs GPU)
...
| Job | Schedule | Leírás |
|-----|----------|--------|
| Email and GitHub Check (daytime) | `0 6-22/2 * * *` CET | 2 óránként email + shared_workspace inbox check |
| Reggeli brief | `30 6 * * *` CET | Napi journal, tegnapi review, email check, brief Tominek Telegramon |
| Heti QC report | `40 7 * * 1` CET | Memory→report/diary/blog minőségellenőrzés + rövid státusz |
| Heti összefoglaló (hétfő) | `0 8 * * 1` CET | Heti report, MEMORY.md frissítés, recovery kit check, GitHub karbantartás |
...
├── BLOG-DESIGN-INSTRUCTION.md  # Astro design spec
├── memory/
...
3. Git config
4. `git clone` shared_workspace → `~/shared_workspace`
...
## 14. VPS Snapshot stratégia

A VPS hosting provider-nél (Contabo/stb.) snapshot készíthető a teljes rendszerállapotról.

**Mikor kell snapshot:**
- Stabil, működő rendszer elérése után (pl. nagy setup befejezése)
- Major változtatás/fejlesztés ELŐTT (biztonsági mentés)
- Új service, infra elem bevezetése után (ha stabil)
- Leoni javasolja, ha érdemes

**Folyamat:**
1. `shutdown -h now` (VPS leállítás)
2. Hosting panelen: snapshot készítés
3. VPS újraindítás
4. Ellenőrzés: minden service fut-e

**Jelenlegi snapshot:** 2026-02-15 (teljes működő rendszer: OpenClaw + ClawGuard + Kanban + email + TTS + STT)
